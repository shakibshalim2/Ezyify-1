...
export const ProductCard = React.memo(({
  ...
  return (
    <Link
      ...
      className="group relative bg-card border border-border rounded-2xl overflow-hidden flex flex-col transition-all duration-200 hover:shadow-xl hover:-translate-y-1 hover:border-border-strong"
    >
      {/* Image wrapper */}
      <div className="relative overflow-hidden aspect-square bg-muted">
        {/* Discount badge */}
        {discount > 0 && (
          <div className="absolute top-2.5 left-2.5 z-10 bg-error text-error-foreground text-[11px] font-bold px-2 py-0.5 rounded-full shadow-sm">
            -{discount}%
          </div>
        )}
        ...
        {product.inStock !== false && (
          <div className="absolute bottom-0 left-0 right-0 translate-y-full group-hover:translate-y-0 transition-transform duration-200">
            <button
              onClick={handleAddToCart}
              className="w-full py-2.5 text-xs font-semibold text-white transition-all duration-150"
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        <p className="text-[11px] text-muted-foreground truncate">{product.seller?.name}</p>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {product.sold > 500 && (
            <span className="text-[10px] font-semibold text-primary ml-auto">{product.sold}+ sold</span>
          )}
        </div>

        <div className="flex items-baseline gap-1.5 mt-auto pt-1">
          <span className={`font-bold text-foreground ${size === 'small' ? 'text-base' : 'text-lg'}`}>
            ${product.price}
          </span>
