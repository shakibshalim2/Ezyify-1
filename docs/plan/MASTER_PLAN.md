...
- 8.7b ◐ **Audit follow‑ups** (`docs/audit/DESIGN_AND_CORRECTNESS_AUDIT.md`) — ☑ Deals → `GET /products?onSale=true` (new API/mock filter), ☑ Wishlist resolves ids through `api.catalog.product`, ☑ Profile fully on core hooks (`useMe/useProfile/useFeed/useLoops/useProducts/useSavedPosts/useToggleFollow/useBlockUser`, share sheet, report/block menu, followers/following links, e2e `profile.spec.ts`), ☑ Live reminders persisted (`lib/reminders.ts`, Notification permission), ☑ `emerald‑*` → `success` tokens, ☑ loop `alt` text. ☐ Remaining local‑fixture pages: Explore, Loops, PostDetail, Stories, Search, SellerStore, LivePage (+ `LoopViewer`, `PostViewer`, `ShoppableProductSheet`) — port to core hooks, then delete `apps/web/src/app/data/*`.
