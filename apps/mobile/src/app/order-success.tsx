...
export default function OrderSuccessScreen() {
  ...
  const { total, orderNumber, method } = useLocalSearchParams<{ total?: string; orderNumber?: string; orderId?: string; method?: string }>();
    ...
    <View style={{ flex: 1, backgroundColor: colors.background, paddingTop: insets.top + 60, paddingHorizontal: 20, paddingBottom: insets.bottom + 20, alignItems: 'center', gap: 20 }}>
      ...
      <Animated.View entering={FadeInDown.delay(150)} style={{ alignItems: 'center', gap: 8 }}>
        ...
        <Text tone="secondary" style={{ textAlign: 'center' }}>
          {orderNumber ? `Order ${orderNumber}` : 'Your order'}{total ? ` · ${formatMoney(money(Number(total) / 100))}` : ''} {paidNow ? 'held safely in escrow' : 'is awaiting payment confirmation'}. We&apos;ll notify you at every step.
        </Text>
      </Animated.View>
      <Animated.View entering={FadeInDown.delay(300)} style={{ width: '100%' }}>
        <Card style={{ gap: 12 }}>
          {[{ icon: 'checkmark-circle', label: paidNow ? 'Paid & escrowed' : 'Awaiting payment', done: paidNow }, { icon: 'cube-outline', label: 'Seller preparing your order', done: false }, { icon: 'bicycle-outline', label: 'On the way', done: false }, { icon: 'home-outline', label: 'Delivered — confirm to release payment', done: false }].map((s, i) => (
            <View key={s.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              {i === (paidNow ? 1 : 0) && <View style={{ backgroundColor: colors.primarySubtle, borderRadius: radius.pill, paddingHorizontal: 8, paddingVertical: 2 }}><Text variant="caption" tone="brand">Now</Text></View>}
            </View>
          ))}
        </Card>
      </Animated.View>
